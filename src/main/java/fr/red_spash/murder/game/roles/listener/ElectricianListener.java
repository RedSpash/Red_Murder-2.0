...
public class ElectricianListener extends GameActionListener {
  ...
  public void inventoryClickEvent(InventoryClickEvent e, Player p, PlayerData playerData, ItemStack itemStack) {
    Role role = playerData.getVisualRole();
      if (!(role instanceof Electrician electrician)) {
          return;
      }
    ...
    ItemMeta itemMeta = itemStack.getItemMeta();
      ...
      if (!itemMeta.getDisplayName().equals(ELECTRICIAN_TRIGGER_ITEM.getItemMeta().getDisplayName())) {
          return;
      }
      ...
      for (Player pl : Bukkit.getOnlinePlayers()) {
        ...
        if (!pdata.isSpectator()) {
          if (pl.getUniqueId() == p.getUniqueId()) {
            ...
            pl.sendTitle("§5§lVous êtes aveuglé!",
                ChatColor.of(electrician.getRoleColor()) + "l'" + electrician.getName() + " vient d'éteindre la lumière!", 0, 20 * 5, 0);
